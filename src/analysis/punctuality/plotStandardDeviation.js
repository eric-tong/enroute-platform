...
export default async function plotStandardDeviation() {
  ...
  const values = [
    ...
    ...grid.map(row =>
      row.map(cell =>
        cell
          ? `${cell.time.toString().padStart(4, "0")} <b>${
              cell.standardDeviation
            }</b>`
          : ""
      )
    )
      ...
      cells: {
        ...
        fill: {
          color: [
            ["white"],
            ...grid.map(row =>
              row.map(cell => getCellColor(cell && cell.standardDeviation))
            )
          ]
        }
      },
  ...
  const layout = {
    title: {
      text: `Standard Deviation from Median Departures`
    },
    autosize: false,
    width: 1600,
    height: 650,
...
async function getScheduledDeparturesGrid() {
  ...
  scheduledDepartures.forEach(departure => {
    const minuteOfDay =
      departure.minuteOfDay + Math.round(medianDelta.get(departure.id) ?? 0);
    const data = {
      name: departure.name,
      time: Math.floor(minuteOfDay / 60) * 100 + (minuteOfDay % 60),
      standardDeviation: standardDeviation.get(departure.id) ?? 0
    };
